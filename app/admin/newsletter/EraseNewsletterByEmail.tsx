...
import { deleteNewsletterSignupsByEmail } from './actions';

export function EraseNewsletterByEmail({ initialEmail = '' }: { initialEmail?: string }) {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (
      !window.confirm(
        `Delete newsletter data associated with "${trimmed}"? This fulfils a right-to-be-forgotten request and cannot be undone.`
      )
    ) {
      return;
    }
    ...
    startTransition(async () => {
      try {
        await deleteNewsletterSignupsByEmail(trimmed);
        setStatus({ type: 'success', message: 'Newsletter records erased.' });
        setEmail('');
      } catch (error) {
        setStatus({
          type: 'error',
          message: error instanceof Error ? error.message : 'Unable to delete records.'
        });
      }
    });
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-2 rounded-2xl border border-neutral-200/70 p-4 dark:border-neutral-800/70">
      <div>
        <label className="text-xs font-semibold uppercase tracking-[0.3em] text-neutral-500 dark:text-neutral-400">
          Data deletion (newsletter)
        </label>
        <p className="mt-1 text-xs text-neutral-500 dark:text-neutral-400">
          Remove newsletter records for a requester. Use after confirming their identity.
        </p>
      </div>
